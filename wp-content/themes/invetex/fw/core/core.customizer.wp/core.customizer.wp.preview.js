...
( function( $ ) {
	...
	if (!invetex_previewer_vars['need_refresh']) {
		var $style = $('#invetex-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="invetex-customizer-css"></'+'style'+'>' ).find('#invetex-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			invetex_set_cookie(invetex_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
