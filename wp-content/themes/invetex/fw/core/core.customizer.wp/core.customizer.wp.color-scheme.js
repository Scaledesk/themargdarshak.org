/* global invetex_color_schemes, invetex_dependencies, Color */
...
( function( api ) {
	...
	if (!invetex_customizer_vars['need_refresh']) {
		for (var i in invetex_color_schemes) {
			cssTemplate[i] = wp.template( 'invetex-color-scheme-'+i );
		}
		cssTemplate['fonts'] = wp.template( 'invetex-fonts' );
	}
	...
	api.bind('ready', function() {
		...
		// Add 'Refresh' button
		if (invetex_customizer_vars['need_refresh']) {
			jQuery('#customize-header-actions .spinner').after('<input type="button" class="button customize-action-refresh" value="'+invetex_customizer_vars['msg_refresh']+'">');
		...
		jQuery('#customize-theme-controls .control-section').each(function () {
			invetex_customizer_wp_check_dependencies(jQuery(this));
		});

		// Update colors
		invetex_customizer_wp_change_color_scheme(api('color_scheme')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		invetex_customizer_wp_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		invetex_customizer_wp_refresh_preview(obj);
	});

	// Check for dependencies
	function invetex_customizer_wp_check_dependencies(cont) {
		"use strict";
		...
		cont.find('.customize-control').each(function() {
			...
			var depend = false;
			for (var fld in invetex_dependencies) {
				if (fld == id) {
					depend = invetex_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('invetex_options_no_use');
				} else {
					jQuery(this).hide().addClass('invetex_options_no_use');
				}
			}
		});
	}
	...
	function invetex_customizer_wp_refresh_preview(obj) {
		"use strict";
		...
		if (id == 'color_scheme') {
			invetex_customizer_wp_change_color_scheme(val);
		} else if (updateCSS) {
			var simple = api('color_settings')()=='simple';
			for (var opt in invetex_color_schemes['original']) {
				if (opt == id) {
					updateCSS = false;
					// Store new value in the color table
					invetex_customizer_wp_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							invetex_customizer_wp_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							invetex_customizer_wp_update_color_scheme('alter_hover', val);
						}
					}
					updateCSS = true;
					processed = true;
			...
			if (!processed) {
				for (var tag in invetex_fonts) {
					for (var prop in invetex_fonts[tag]) {
						if (prop=='title' || prop=='description') continue;
						if (tag+'-'+prop == id) {
							processed = true;
							// Store new value in the fonts table
							invetex_customizer_wp_update_fonts(tag, prop, val);
							break;
						}
					}
					if (processed) break;
			...
			// Refresh CSS
			if (processed) invetex_customizer_wp_update_css();
		}
	...
	// Store new value in the color table
	function invetex_customizer_wp_update_color_scheme(opt, value) {
		"use strict";
		invetex_color_schemes[api('color_scheme')()][opt] = value;
		api('scheme_storage').set(invetex_serialize(invetex_color_schemes))
	}
	

	// Change color scheme - update colors and generate css
	function invetex_customizer_wp_change_color_scheme(value) {
		"use strict";
		updateCSS = false;
		for (var opt in invetex_color_schemes[value]) {
			if (api(opt) == undefined) continue;
			api( opt ).set( invetex_color_schemes[value][opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', invetex_color_schemes[value][opt] )
				.wpColorPicker( 'defaultColor', invetex_color_schemes[value][opt] );
		}
		updateCSS = true;
		invetex_customizer_wp_update_css();
	}

	// Store new value in the fonts table
	function invetex_customizer_wp_update_fonts(tag, prop, value) {
		"use strict";
		invetex_fonts[tag][prop] = value;
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function invetex_customizer_wp_update_css() {
		"use strict";

		if (!updateCSS || invetex_customizer_vars['need_refresh']) return;

		var css = '';
		
		// Add color styles
		for (var scheme in invetex_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in invetex_color_schemes[scheme]) {
				if (i=='title') continue;
				colors[i] = invetex_color_schemes[scheme][i];
			}
		...
		// Make theme specific colors and tints
		if (window.invetex_customizer_wp_add_theme_fonts) fonts = invetex_customizer_wp_add_theme_fonts(fonts);
