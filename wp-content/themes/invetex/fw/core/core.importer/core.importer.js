...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		INVETEX_STORAGE['importer_error_messages'] = '';
		INVETEX_STORAGE['importer_ignore_errors'] = true;
		invetex_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function invetex_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: INVETEX_STORAGE['ajax_nonce'],
		action: 'invetex_importer_start_import',
	...
	jQuery.post(INVETEX_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '' || INVETEX_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '') 
				INVETEX_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				invetex_importer_do_action(steps, idx);
			} else {
				if (INVETEX_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + INVETEX_STORAGE['importer_error_msg'] + '</h4>' + INVETEX_STORAGE['importer_error_messages']);
				} else {
