...
if (invetex_get_custom_option("show_post_related") == 'yes' && $post_data['post_type'] != 'page') {
	...
	if (!empty($post_data['post_terms'][$post_data['post_taxonomy']]->terms_ids))
		$args = invetex_query_add_posts_and_cats($args, '', $post_data['post_type'], $post_data['post_terms'][$post_data['post_taxonomy']]->terms_ids, $post_data['post_taxonomy']);
	$args = invetex_query_add_sort_order($args, invetex_get_custom_option('post_related_sort'), invetex_get_custom_option('post_related_order'));
	...
	$args = apply_filters('invetex_filter_related_posts_args', $args, $post_data);

	$recent_posts = get_posts( $args, OBJECT );

	$number = is_array($recent_posts) ? count($recent_posts) : 0;
	if ($number > 0) {
		$columns = max(1, invetex_get_custom_option('post_related_columns'));
		if ($columns < 2) {
			if ($number < 3)
				$columns = 3;
			else
				invetex_enqueue_slider();	// Add slider and scrollbar scripts
		}
		$need_dummy = false;
		?>
		<section class="related_wrap<?php echo ((int) $columns > 1 ? '' : ' scroll_wrap') . esc_attr(invetex_get_template_property('related', 'container_classes')); ?>">

			<?php if ($need_wrap) invetex_open_wrapper('<div class="content_wrap">'); ?>
			
			<h2 class="section_title"><?php echo apply_filters('invetex_filter_related_posts_title', esc_html__('Related Posts', 'invetex'), $post_data['post_type']); ?></h2>
			...
			<?php } else if (invetex_get_template_property('related', 'need_columns')) { ?>
						...
						foreach ($recent_posts as $recent) {
							...
							invetex_show_post_layout(
								array(
									...
									'number' => $i,
									'add_view_more' => false,
									'posts_on_page' => invetex_get_custom_option('post_related_count'),
									'columns_count' => $columns,
									'strip_teaser' => false,
									'sidebar' => !invetex_param_is_off(invetex_get_custom_option('show_sidebar_main')),
									'content' => invetex_get_template_property('related', 'need_content'),
									'terms_list' => invetex_get_template_property('related', 'need_terms')
								),
								null,
								$recent
							);
			...
			<?php if ($need_wrap) invetex_close_wrapper(); ?>

		</section>
		<?php
	}
	if ($body_style!='fullscreen' && !$sidebar_present) invetex_open_all_wrappers();
}
