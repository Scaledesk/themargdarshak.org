...
class invetex_widget_recent_news extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'invetex'));
		parent::__construct( 'invetex_widget_recent_news', esc_html__('Themerex - Recent News', 'invetex'), $widget_ops );
	}
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php esc_html_e('Widget title:', 'invetex'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Block title:', 'invetex'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widgets_param_fullwidth" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('subtitle')); ?>"><?php esc_html_e('Block subtitle:', 'invetex'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style:', 'invetex'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number of posts to be displayed (9 max):', 'invetex'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('featured')); ?>"><?php esc_html_e('Number of featured posts:', 'invetex'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('columns')); ?>"><?php esc_html_e('Number of columns:', 'invetex'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show categories:', 'invetex'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show', 'invetex'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_0"><?php esc_html_e('Hide', 'invetex'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show counters:', 'invetex'); ?></label><br />
			...
			if (invetex_param_is_on($show_categories) && !empty($cat_list)) {
				...
				if (is_array($cat_list) && count($cat_list) > 0) {
					$output .= '<a href="' . esc_url( $category == 0 
						...
						: get_category_link($category) ) . '" class="sc_recent_news_header_category_item">'.esc_html__('All News', 'invetex').'</span>';
					$number = 0;
					$number_max = 3;
					foreach ($cat_list as $cat) {
						$number++;
						if ($number == $number_max)
							$output .= '<span class="sc_recent_news_header_category_item sc_recent_news_header_category_item_more">'.esc_html__('More', 'invetex')
...
if (!function_exists('invetex_widget_recent_news_reg_shortcodes_vc')) {
	...
	function invetex_widget_recent_news_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Recent News", 'invetex'),
				"description" => wp_kses_data( __("Insert recent news list", 'invetex') ),
				...
				"params" => array(
					array(
						"param_name" => "style",
						"heading" => esc_html__("List style", 'invetex'),
						"description" => wp_kses_data( __("Select style to display news list", 'invetex') ),
						"class" => "",
						"admin_label" => true,
						"std" => 'news-magazine',
					...
					array(
						...
						"description" => wp_kses_data( __("Title for the widget (fill this field only if you want to use shortcode as widget)", 'invetex') ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"heading" => esc_html__("Show categories", 'invetex'),
						"description" => wp_kses_data( __("Show categories in the shortcode's header", 'invetex') ),
						"class" => "",
					...
					array(
						...
						"heading" => esc_html__("Show counters", 'invetex'),
						"description" => wp_kses_data( __("Show counters (comments and views) in the shortcode's footer", 'invetex') ),
						'dependency' => array(
							'element' => 'style',
							'value' => 'news-magazine'
						),
					...
					array(
						"param_name" => "cat",
						"heading" => esc_html__("Category", 'invetex'),
						"description" => wp_kses_data( __("Select category to show news. If empty - select news from any category or from IDs list", 'invetex') ),
						"group" => esc_html__('Query', 'invetex'),
						"class" => "",
					...
					array(
						"param_name" => "count",
						"heading" => esc_html__("Total posts", 'invetex'),
						"description" => wp_kses_data( __("How many posts will be displayed? If used IDs - this parameter ignored. (9 max)", 'invetex') ),
						"admin_label" => true,
						"group" => esc_html__('Query', 'invetex'),
						"class" => "",
						"value" => "3",
					...
					array(
						"param_name" => "featured",
						"heading" => esc_html__("Featured posts", 'invetex'),
						"description" => wp_kses_data( __("How many posts will be displayed as featured?", 'invetex') ),
						"admin_label" => true,
						"group" => esc_html__('Query', 'invetex'),
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"description" => wp_kses_data( __("How many columns use to show news list", 'invetex') ),
						"group" => esc_html__('Query', 'invetex'),
						"admin_label" => true,
						'dependency' => array(
							'element' => 'style',
					...
					array(
						"param_name" => "ids",
						"heading" => esc_html__("List IDs", 'invetex'),
						"description" => wp_kses_data( __("Comma separated list of post's ID. If set - parameters above (category, count, order, etc.) are ignored!", 'invetex') ),
						"group" => esc_html__('Query', 'invetex'),
						"class" => "",
						"value" => "",
						"type" => "textfield"
